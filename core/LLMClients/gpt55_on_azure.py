"""
GPT-5.5 on Azure OpenAI LLM client implementation.

Uses the ``openai`` SDK's ``AzureOpenAI`` client pointed at an
Azure OpenAI endpoint.  The wire protocol is identical to GPT-5,
so this adapter mirrors :class:`GPT5OnAzureClient` with a different
...
class GPT55OnAzureClient(LLMClient):
    """Adapter for GPT-5.5 models hosted on Azure OpenAI."""

    PROVIDER = "GPT-5.5 on Azure"
