...
class ProjectManager:
    ...
    def _patch_orphaned_tool_use(self, msgs: list) -> None:
        ...
        while i < len(msgs):
            ...
            _log.warning(
                "[PM] Repairing %d orphaned tool_use block(s) at msg index %d",
                len(tool_ids), i,
            )
            repair = [
    ...
    def _run_tool_loop(self, msgs: list) -> None:
        ...
        while not self._stop.is_set() and not self._cancel.is_set():
            ...
            if bg_notifs:
                bg_text = f"<background-results>\n{''.join(bg_notifs)}\n</background-results>"
                # Avoid consecutive user messages (previous may be tool_results)
                if msgs and msgs[-1].get("role") == "user":
                    msgs.append({"role": "assistant", "content": "Acknowledged."})
                msgs.append({"role": "user", "content": bg_text})
